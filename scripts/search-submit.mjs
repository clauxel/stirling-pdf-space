...
function cloudflareHeaders() {
  ...
  const key = envOrKey('CLOUDFLARE_API_KEY')
  const email = envOrKey('CLOUDFLARE_EMAIL', ['CLOUDFLARE_API_EMAIL'])
...
async function ensureGoogleTxtRecord(zoneId, token) {
  const comment = 'Stirling PDF Space Google site verification'
...
function bingKey() {
  const key = envOrKey('BING_WEBMASTER_API_KEY', ['BING_WEEMASTER_API_EY', ['codex-bing-webmaster-api-key', 'bing']])
...
async function submitBing(urls) {
  return {
    siteUrl,
    sitemapUrl,
    bingAuthFile: `${siteUrl}BingSiteAuth.xml`,
